...
//Core::Transformation::NoiseRemover::NoiseRemover() //: middle_color((IColor::binary_color::turn_on + IColor::binary_color::turn_off) / 2)
//{
//}

void Core::Transformation::NoiseRemover::execute(Objects::Visual::IImage* image)
{
	unsigned char turn_on = static_cast<unsigned char>(IColor::binary_color::turn_on);
	unsigned char turn_off = static_cast<unsigned char>(IColor::binary_color::turn_off);
	...
	for (int i = 1; i < image->rows() - 1; i++)
	{
		for (int j = 1; j < image->cols() - 1; j++)
		{
			...
			if(current_color->brightness() == turn_off)
			{
				std::vector<IColor*> neighbours = image->get_neighbours(i, j, turn_on);
			...
			{
				std::vector<IColor*> neighbours = image->get_neighbours(i, j, turn_off);
				if (neighbours.size() == 5)
				{
					Color color(turn_off);
					image->set(i, j, &color);
				}
			}
...
//unsigned char Core::Transformation::NoiseRemover::get_dominant_color(std::vector<IColor*> colors)
//{
